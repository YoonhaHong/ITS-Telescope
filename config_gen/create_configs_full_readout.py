...
def _create_output_name(prefix, default, half_unit, region, vcasb):
    if prefix is None: output_name = f"{default}_{half_unit}_region{region}_VCASB{vcasb}"
    else: output_name = f"{prefix}_{default}_{half_unit}_Rregion{region}_VCASB{vcasb}"
    return output_name
...
def create_eudaq2_configs(eudaq2_template, device_name, vcasb_list, region, vcasb_default, half_unit=None, nevents=None, region_enabled=None, ibias=None, ibiasn=None, idb=None, ireset=None, vshift=None, vcasn=None, bandgap_trim=None, output_dir=None, settings_dir=None, output_name_prefix=None):
    ...
    # for r in REGIONS: 
    #     if ibias is not None: confGen.set_value(f'tb_region{r}_IBIAS', ibias, 'Producer.MOSS_3')
    # for r in REGIONS: 
    #     if ibias is not None: confGen.set_value(f'bb_region{r}_IBIAS', ibias, 'Producer.MOSS_3')
    # for r in REGIONS: 
    #     if ibiasn is not None: confGen.set_value(f'tb_region{r}_IBIASN', ibiasn, 'Producer.MOSS_3')
    # for r in REGIONS: 
    #     if ibiasn is not None: confGen.set_value(f'bb_region{r}_IBIASN', ibiasn, 'Producer.MOSS_3')
    # for r in REGIONS: 
    #     if idb is not None: confGen.set_value(f'tb_region{r}_IDB', idb, 'Producer.MOSS_3')
    # for r in REGIONS: 
    #     if idb is not None: confGen.set_value(f'bb_region{r}_IDB', idb, 'Producer.MOSS_3')
    # for r in REGIONS: 
    #     if ireset is not None: confGen.set_value(f'tb_region{r}_IRESET', ireset, 'Producer.MOSS_3')
    # for r in REGIONS: 
    #     if ireset is not None: confGen.set_value(f'bb_region{r}_IRESET', ireset, 'Producer.MOSS_3')
    # for r in REGIONS: 
    #     if vshift is not None: confGen.set_value(f'tb_region{r}_VSHIFT', vshift, 'Producer.MOSS_3')
    # for r in REGIONS: 
    #     if vshift is not None: confGen.set_value(f'bb_region{r}_VSHIFT', vshift, 'Producer.MOSS_3')
    # for r in REGIONS: 
    #     if vcasn is not None: confGen.set_value(f'tb_region{r}_VCASN', vcasn, 'Producer.MOSS_3')
    # for r in REGIONS: 
    #     if vcasn is not None: confGen.set_value(f'bb_region{r}_VCASN', vcasn, 'Producer.MOSS_3')
    ...
    # sets vcasb for all planes except dut on choosen range of values
    # for r in REGIONS:
    #     confGen.set_value(f'Region{r}_VCASB', vcasb_default, 'Producer.MOSS_0')
    #     confGen.set_value(f'Region{r}_VCASB', vcasb_default, 'Producer.MOSS_1')
    #     confGen.set_value(f'Region{r}_VCASB', vcasb_default, 'Producer.MOSS_2')
    #     confGen.set_value(f'Region{r}_VCASB', vcasb_default, 'Producer.MOSS_4')
    #     confGen.set_value(f'Region{r}_VCASB', vcasb_default, 'Producer.MOSS_5')
    #     confGen.set_value(f'Region{r}_VCASB', vcasb_default, 'Producer.MOSS_6')
    
    # sets the vcasb of DUT on fixed value
    # confGen.set_value(f'Region{0}_VCASB', 5, 'Producer.MOSS_3') 
    ...
    for vcasb in vcasb_list:
        ...
        output_name = _create_output_name(prefix=output_name_prefix, default='EUDAQ2_conf', half_unit=half_unit, region=region, vcasb=vcasb)
...
def create_moss_testing_configs(moss_testing_template, device_name, vcasb_list, region, vcasb_default, half_unit=None, ts_config_path=None, top_result_dir=None, region_enabled=None, ibias=None, ibiasn=None, idb=None, ireset=None, vshift=None, vcasn=None, bandgap_trim=None, output_dir=None, settings_dir=None, output_name_prefix=None):
    ...
    for vcasb in vcasb_list:
        ...
        output_name = _create_output_name(prefix=output_name_prefix, default='moss_testing_conf', half_unit=half_unit, region=region, vcasb=vcasb)
        file_path = confGen.save_config(file_name=output_name)
        config_paths.append(file_path)
