...
parser = argparse.ArgumentParser(description="Endless current readout")
...
if apts.is_chip_powered() is False:
    logging.info("APTS was off --> turning ON")
    apts.power_on()


of = open(os.path.join(args.output_dir,fname+"_currents.dat"),'w')
of.write("time\tIa (mA)\n")
print('#time\tIa (mA)')
...
try:
    while True:
    	...
    	Ia = apts.read_isenseA()
    	print(f"{unix_time}\t{Ia}")
    	of.write(f"{unix_time}\t{Ia}\n")
    	sleep(2)
