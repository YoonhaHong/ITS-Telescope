...
def analysis_vh_scan(args):
    ...
    for vbb_dir in tqdm(scanned_vbb, desc="Vbb"):
        ...
        raw_data_file = glob(f"{vbb_dir}/opamp_vh_scan_*.raw")[0]
        ...
        if args.gain_calibration_path is not None:
            data_calibration_file = glob(os.path.join(args.gain_calibration_path, f"{vbb}/opamp_gain_*_*_analysed.npz"))[0]
            data_calibration = np.load(data_calibration_file)
            output_file_suffix = "_calibrated"
        for i in tqdm(range(waveforms.shape[0]), leave=False, desc="Waveform"):
            ...
            for p in range(waveforms.shape[1]):
                ...
                graph, resampled_graph, baseline, baseline_rms, baseline_noise_point, underline, underline_rms, underline_noise_point, t0_bin, CFD_range, CFD_times = au.analyse_waveform(adc_signal=adc_waveform,\
                    ...
                    signal_extraction_method=args.signal_extraction_method, resample_waveform=args.resample_waveform, integration_time=args.integration_time, gain_calibration_data=data_calibration)
                if (baseline-underline > 1.5) and (np.all(CFD_times) != -999.):
                    if args.debug_plot:
                        au.plot_waveform(graph=graph, baseline=baseline, underline=underline, t0_bin=t0_bin, scope_axis_dict=scope_ADC_to_mV_json, CFD_times=CFD_times, CFD_search_limit=CFD_range[1],\
                            scope_channel=scope_channel, resampled_graph=resampled_graph, vbb=vbb[-3:], vh=vh, trigger=int(i%ntrg))
                    df_waveforms = au.write_waveforms_dataframe(dataframe=df_waveforms, script_name=os.path.basename(__file__), parameter=vh, scope_channel=int(scope_channel), trigger=int(i%ntrg),\
        ...
        df_waveforms.to_csv(os.path.join(vbb_dir, f"waveforms{output_file_suffix}.csv"), sep='|', index=False)  # save file
        df_statistics = au.make_statistics_dataframe(script_name=os.path.basename(__file__))
        df_statistics = au.process_waveforms_dataframe(waveforms_dataframe=df_waveforms, statistics_dataframe=df_statistics, script_name=os.path.basename(__file__), value_range=vh_range,\
            channel_to_pixel_dictionary=pixel_dict, make_control_plots=args.control_plots, baseline_and_underline_evaluation_points=\
            (args.baseline_evaluation_interval,args.baseline_first_time_before_t0,args.underline_evaluation_interval,args.underline_first_time_after_t0), output_plot_dir=vbb_dir, output_file_suffix=output_file_suffix)
        df_statistics.to_csv(os.path.join(vbb_dir, f"vh_scan_statistics{output_file_suffix}.csv"), sep='|', index=False)  # save file
        with open(os.path.join(vbb_dir, "vh_scan_processing_arguments.json"),'w') as file_handle:
            json.dump(vars(args), file_handle, indent=4)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="APTS OPAMP routine to process data acquired by pulsing with varying vh.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    parser.add_argument('--gain_calibration_path', '-g', default=None, help='Root directory with gain calibration files. If not provided, gain calibration of measured signals is NOT performed.')
    parser.add_argument('--control_plots', action='store_true', help='Produce signal amplitude and falltime histograms for each vh.')
