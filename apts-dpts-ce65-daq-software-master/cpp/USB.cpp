...
int TUSBBoard::SetDeviceInfo(){
	...
	if ((fVendorId == 0x04b4) && (fProductId == 0x00F0 || fProductId == 0x00F1))
	  {
	    fUsbChipType="FX3";
	    printf("CHIP type: FX3\n");
	  }
	/*       	else if ((fVendorId == 0x1556) && (fProductId == 0x01b8))
	  {
	    fUsbChipType="FX3";
	    printf("CHIP type: FX3[MM]\n");
	  }
	*/
	else if ((fVendorId == 0x04b4) && (fProductId == 0x1004))
	  {
	    fUsbChipType="FX2";
	    printf("CHIP type: FX2 \n");
	  }
	else {
        fUsbChipType="OTHER";
...
int TUSBEndpointControl::GetConfiguration() {
    ...
    // DBG std::cout << "Received data 0x" << std::hex << (int) data[0] << std::dec << std::endl;
    return size;
